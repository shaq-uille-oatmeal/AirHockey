...
class sphere{
    ...
    update_paddle_computer(pucks){
        ...
        if(closest_puck.pos.x<=canvas_dim.x*0.5){                                     //CONDITION to FOLLOW PUCK ONLY ALONG Y (if puck in player half)
            ...
            this.vx=v*Math.cos(theta);
            this.vy=v*Math.sin(theta);                                                  //follow cat and mouse type
            if (closest_puck.vx<0 && closest_puck.pos.x>0.75*canvas_dim.x){             //if vx away from goal and puck >0.75*window width then runs away
                this.vx=-this.vx;  
                this.vy=-this.vy;
            }
            else if (closest_puck.vx>0 && closest_puck.pos.x>=this.pos.x){              //if caught out of position
                if (closest_puck.pos.y>=canvas_dim.y/2){                                //If puck in lower half
                    theta= Math.atan2(canvas_dim.y/2+canvas_dim.y/6-this.pos.y,canvas_dim.x-this.radius-this.pos.x);        //Rush to near post
                    this.vx=v*Math.cos(theta);
                    this.vy=v*Math.sin(theta); 
                }
                else if (closest_puck.pos.y<canvas_dim.y/2){                            //if puck in upper half
                    theta= Math.atan2(canvas_dim.y/2-canvas_dim.y/6-this.pos.y,canvas_dim.x-this.radius-this.pos.x);        //Rush to near post
                    this.vx=v*Math.cos(theta);
                    this.vy=v*Math.sin(theta);
                }
